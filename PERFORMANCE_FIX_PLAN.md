...
32. BGMの配信用圧縮版を用意する。
33. HUD波形と地図リサージュは別ループを作らず、音量メーターの24fpsループを共有する。
34. リサージュは1フレーム最大512点に制限し、L=X・R=Yのraw-XYを維持する。

### Data And Accessibility

35. 完全に空のSpreadsheet行を描画前に除外する。
36. `null / null` をArchive Wheelへ出さない。
37. 起動オーバーレイをキーボード操作可能にする。
38. フォーカス表示を主要操作へ追加する。
39. 動きの削減設定を尊重する。
40. 小さすぎる文字と低コントラスト表示を将来調整する。

### Verification And Maintenance

41. DOM数、Tooltip数、初期画像数を回帰確認する。
42. ホバー時に `selectedWorkId` が変わらないことをテストする。
43. 折り畳み中にArchive Wheel全件のスタイルが更新されないことを確認する。
44. build、キーボード、タッチ、フィルター、音声、外部リンクを確認する。
45. 性能上の設計判断をWORKLOGとBUGFIX_LOGへ残す。
